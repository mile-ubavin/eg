  ...
  it.only("Update personal data - all fields", function () {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      cy.get(tag)
        .find(".mat-option")
        .then((matOptionItems) => {
          const totalNumbersOfDropdownItems = Cypress.$(matOptionItems).length; //Total number of items visible in dropdown
          expect(matOptionItems).to.have.length(totalNumbersOfDropdownItems);
          //Random selected item from dropdown
          const itemToSelect = matOptionItems
            .eq(Math.floor(Math.random() * totalNumbersOfDropdownItems))
            .text();
    ...
    for (let i = 0; i < 2; i++) {
      ...
      cy.get(".iti__country-list")
        ...
        // .find(".iti__country")
        .then((a) => {
          const totalNumberOfCountriesInDropdown = Cypress.$(a).length; //Total number of items visible in dropdown
          expect(a).to.have.length(totalNumberOfCountriesInDropdown);
          const idRandomSelectedCountry = Math.floor(
            Math.random() * totalNumberOfCountriesInDropdown
          );
          cy.log("RANDOM NUMBER:", idRandomSelectedCountry);
          a.eq(idRandomSelectedCountry).trigger("click");
        });
    }
      ...
      .then((a) => {
        ...
        for (let index = 0; index < inputCount - 1; index++) {
          ...
          if (index == 2 || index == 4) {
            ...
            cy.log(
              "Display random selected country number - ",
              idRandomSelectedCountry
            );
            // cy.log("itemToSelect1", itemToSelect1);
            if (idRandomSelectedCountry == 0) {
              cy.get(
                ".chameleon-form>.two-col-grid-row>.two-col-grid-controls>.form-field"
              )
                .eq(index)
    ...
    // cy.get("button>.button__footer>.button__icon").click();
    // cy.wait(2000);
  }); //end it
