...
describe("Login, Update personal data, Logout", () => {
  ...
  it.only("Update personal data - all fields", function () {
        ...
        cy.get("form.chameleon-form>div>.two-col-grid-label")
          ...
          .then(() => {
            for (let index = 0; index < listingCount; index++) {
              ...
              cy.log("then log", labeName[index]);
            }
          });
      });
    ...
    //         //let dropdownItemText = []; //linkText array
    //         // const item = $el.text();
    //         //dropdownItemText[index] = $el.text();
    //         //cy.log("count", listingCount);
    //         //cy.log("TXT------->", dropdownItemText[index]);
    ...
    //Get total numbers of input fields labels
    cy.get(".chameleon-form>.two-col-grid-row>.two-col-grid-controls")
      ...
      .then((a) => {
        ...
        for (let index = 0; index < inputCount - 1; index++) {
          cy.fixture("example.json").then((a) => {
            cy.get(
          ...
          // if (index == 0 || index == 1) {
          //   cy.fixture("footer_url.json").then((user) => {
          //     cy.get(
          //       ".chameleon-form>.two-col-grid-row>.two-col-grid-controls>.form-field"
          //     )
          //       .eq(0)
          ...
          //     if (itemToSelect1 !== "Austria (Österreich)") {
